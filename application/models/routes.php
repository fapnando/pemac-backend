...
class Routes extends CI_Model {
	...
	{
		return $this->db->get_where($this->table, array('id'=>$id))->row()->slug;
	}
	...
	function save($route)
	{
		if(!empty($route['id']))
		{
			...
			$this->db->update($this->table, $route);
	...
	function get_by_slug($slug){
		return $this->db->get_where($this->table, array('slug'=>$slug))->row()->id;
	}
